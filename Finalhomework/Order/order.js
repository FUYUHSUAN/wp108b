...
function storeData(){
    ...
    if(food_type == 1)localStorage.getItem('pizza',qty);//getItem作為storage的參數或鍵名，會將鍵名加到存處中，如果鍵名已存在則更新其值。
    if(food_type == 2)localStorage.getItem('sandwich',qty);
    if(food_type == 3)localStorage.getItem('dessert',qty);
    if(food_type == 4)localStorage.getItem('juice',qty);
    if(food_type == 5)localStorage.getItem('steak',qty);
}
